...
class CoveredCallStrategy(Strategy):
    ...
    def _write_new_call(
        ...
        valid_calls = [
            ...
            if (call.expiration.replace(tzinfo=pytz.UTC) - timestamp).days
        ...
        if success:
            ...
            self.logger.info(
                f"Sold call for {ticker} @ strike {selected_call.strike} expiring {selected_call.expiration.date()}"
            )

    def _manage_existing_call(
        self, ticker: str, current_price: float, timestamp: datetime
    ) -> None:
        ...
        if timestamp > call.expiration:
            for pos_ticker, position in self.portfolio.positions.items():
                self.logger.info(
                    f"  {pos_ticker}: {type(position.instrument).__name__}, {position.quantity} units"
                )

            self.active_calls.pop(ticker)

            if any(
                isinstance(pos.instrument, Stock)
                for pos in self.portfolio.positions.values()
            ):
                self._write_new_call(ticker, current_price, timestamp)
            return
        ...
        if dte <= self.roll_dte_threshold:
            ...
            for pos in self.portfolio.positions.values():
                if (
                    isinstance(pos.instrument, Stock)
                    and pos.instrument.ticker == ticker
                ):
                    stock_position = pos
                    break
            ...
            success = self.engine.buy_option(option=call, quantity=contracts)
