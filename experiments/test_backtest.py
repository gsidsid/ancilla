...
class VolatilityDivergenceStrategy(Strategy):
    ...
    def __init__(self, data_provider, lookback_days=30, vol_threshold=0.15,
        ...
        """
        super().__init__(data_provider, name="vol_divergence")
...
def run_volatility_backtest(api_key: str, tickers: list, start_date: datetime,
                          end_date: datetime, initial_capital: float = 100000):
    """Run a backtest of the volatility divergence strategy."""

    # Initialize data provider
    data_provider = PolygonDataProvider(api_key)

    # Create strategy instance
    strategy = VolatilityDivergenceStrategy(
        data_provider,
        lookback_days=30,
        vol_threshold=0.15,
        max_positions=5,
    ...
    # Set up option chain parameters with more conservative filters
    option_params = {
        'expiration_range_days': 45,    # Reduced from 60
        'min_days': 7,
        'min_volume': 5,               # Reduced from 10
        'delta_range': (0.1, 0.9)      # Wider range
    }
    ...
    equity_curve['equity'].plot(title='Strategy Equity Curve')
...
if __name__ == "__main__":
    ...
    results = run_volatility_backtest(os.getenv("POLYGON_API_KEY") or "your-api-key", tickers, start_date, end_date)
