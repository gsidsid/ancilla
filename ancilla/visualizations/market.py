...
from ancilla.formulae.volatility import create_volatility_surface, estimate_liquidity_multiplier
...
class MarketVisualizer:
    ...
    def __init__(self, data_provider):
        ...
        self.logger.info("Initialized MarketVisualizer with provider: %s", type(data_provider).__name__)
    ...
    def _log_dataframe_stats(self, df: pd.DataFrame, prefix: str = "") -> None:
        ...
        self.logger.debug(f"{prefix} DataFrame stats:\n{json.dumps(stats, indent=2, default=str)}")
    ...
    def _prepare_surface_data(
        ...
        self.logger.info(f"Preparing surface data for {len(options_data)} options contracts")

        # Log input data statistics for debugging flat surface
        iv_stats = {
            "raw_iv_range": (
                min(opt.implied_volatility for opt in options_data if opt.implied_volatility is not None),
                max(opt.implied_volatility for opt in options_data if opt.implied_volatility is not None)
            ),
        }
        self.logger.debug(f"Input IV statistics:\n{json.dumps(iv_stats, default=str, indent=2)}")

        dfs = []
        for contract_type in ['call', 'put']:
            options = [opt for opt in options_data if opt.contract_type == contract_type]
            if not options:
            ...
            self.logger.debug(f"{contract_type} pre-weight stats:\n{json.dumps(pre_weight_stats, indent=2)}")
            ...
            df.loc[moneyness_mask, 'weight'] = (df.loc[moneyness_mask, 'weight'] * 1.5).astype(int)
            ...
            self.logger.debug(f"{contract_type} post-weight stats:\n{json.dumps(post_weight_stats, indent=2)}")
        ...
        final_stats = {
            ...
            "moneyness_range": (float(result['moneyness'].min()), float(result['moneyness'].max())),
            "tte_range": (float(result['tte'].min()), float(result['tte'].max()))
        }
        self.logger.debug(f"Final surface data statistics:\n{json.dumps(final_stats, indent=2)}")
    ...
    def plot_volatility_surfaces(
        ...
        self.logger.debug(f"Animation parameters: start={start_date}, end={end_date} expiry_range={expiration_range} moneyness_range={moneyness_range}")
        ...
        try:
            ...
            def fetch_surface_for_date(date: datetime) -> Tuple[str, Optional[Tuple]]:
                ...
                try:
                    ...
                    options_data = self.provider.get_options_chain(ticker, reference_date=date, min_days=min_days, expiration_range_days=max_days)
                    if not options_data:
                    ...
                    n_money_points = max(50, int((moneyness_range[1] - moneyness_range[0]) * 100))
                    n_tte_points = max(50, int((max_days - min_days) * 0.75))
                    actual_max_expiry = float(df['tte'].max())
                    money_points = np.linspace(moneyness_range[0], moneyness_range[1], n_money_points)
                    ...
                    surface_input_stats = {
                        "num_points": len(df),
                        "strike_range": (float(df['strike'].min()), float(df['strike'].max())),
                        "iv_range": (float(df['iv'].min()), float(df['iv'].max())),
                        "tte_range": (float(df['tte'].min()), float(df['tte'].max()))
                    }
                    self.logger.debug(f"Surface creation inputs:\n{json.dumps(surface_input_stats, indent=2)}")
                    ...
                    if stats["z_std"] < 0.001:
                        self.logger.warning(f"WARNING: Nearly flat surface detected for {date}")
                        self.logger.debug(f"Surface statistics:\n{json.dumps(stats, indent=2)}")
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = [executor.submit(fetch_surface_for_date, date) for date in date_range]
            ...
            evolution_stats = {
                ...
                "processing_times": {
                    "mean": float(np.mean([s["processing_time"] for s in surface_stats_over_time])),
                    "max": float(np.max([s["processing_time"] for s in surface_stats_over_time]))
                },
                "surface_stability": {
                    "mean_std": float(np.mean([s["z_std"] for s in surface_stats_over_time])),
                    "std_of_means": float(np.std([s["z_mean"] for s in surface_stats_over_time]))
                }
            }
            ...
            fig.update_layout(
                ...
                sliders=[{
                    ...
                    'steps': [{
                        ...
                        'label': frame.name.split('_')[0],  # Only show the date part in slider
                        'method': 'animate'
                    } for frame in frames]
                }],
                height=800,
            ...
            self.logger.info(f"Animation creation completed in {total_time:.2f} seconds")
            return fig
    ...
    def plot_option_chain(
        ...
        try:
            ...
            if expiry_filter:
                ...
                self.logger.debug(f"Filtered to {len(df)} options after applying expiry filter")
            ...
            fig = make_subplots(
                ...
                subplot_titles=('Option Prices', 'Volume', 'Greeks') if plot_greeks else ('Option Prices', 'Volume')
            )
            ...
            for opt_type in ['call', 'put']:
                ...
                stats = {
                    "count": len(type_data),
                    "price_range": (float(type_data['ask'].min()), float(type_data['ask'].max())),
                    "volume_range": (int(type_data['volume'].min()), int(type_data['volume'].max())),
                    "strike_range": (float(type_data['strike'].min()), float(type_data['strike'].max()))
                }
                if plot_greeks:
                    stats.update({
                        "delta_range": (float(type_data['delta'].min()), float(type_data['delta'].max())),
                        "gamma_range": (float(type_data['gamma'].min()), float(type_data['gamma'].max()))
                    })

                self.logger.debug(f"{opt_type.upper()} options statistics:\n{json.dumps(stats, indent=2)}")
            ...
            self.logger.info(f"Option chain visualization completed in {time.time() - start_time:.2f} seconds")
            return fig
    ...
    def plot_technical_analysis(
        ...
        start_time = time.time()
        self.logger.info(f"Creating technical analysis for {ticker} with indicators: {indicators}")
        ...
        try:
            ...
            self.logger.info(f"Technical analysis visualization completed in {time.time() - start_time:.2f} seconds")
            return fig
    ...
    def plot_liquidity_analysis(
        self,
        ticker: str,
        options_data: Optional[List[OptionData]] = None
    ) -> Optional[go.Figure]:
        ...
        try:
            ...
            self.logger.debug(f"Liquidity statistics:\n{json.dumps(liquidity_stats, indent=2)}")
            ...
            df['liquidity_score'] = df.apply(
                lambda row: estimate_liquidity_multiplier(
                    ...
                    time_to_expiry=(row['expiration'] - datetime.now(self.utc_tz)).total_seconds() / (365.25 * 24 * 3600)
                ),
                axis=1
            )
            ...
            score_stats = {
                ...
                "score_range": (float(df['liquidity_score'].min()), float(df['liquidity_score'].max()))
            }
            self.logger.debug(f"Liquidity score statistics:\n{json.dumps(score_stats, indent=2)}")
            ...
            self.logger.info(f"Liquidity analysis visualization completed in {time.time() - start_time:.2f} seconds")
            return fig
