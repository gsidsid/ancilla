...
class Portfolio:
    ...
    def __init__(self, name: str, initial_capital: float, enable_naked_options: bool = True):
    ...
    def open_position(
        ...
        if instrument.is_option and quantity < 0:  # Short options
            if instrument.instrument_type == InstrumentType.CALL_OPTION:
                ...
                share_check = self.positions[instrument.underlying_ticker].quantity < required_shares if instrument.underlying_ticker in self.positions else True
                warning_msg = f"Insufficient shares for covered call: need {required_shares}, "
            else:
                # CASH SECURED PUT
            ...
            if share_check:
                ...
                if not self.enable_naked_options:
                    self.logger.get_logger().error("Naked options are not enabled. This position will not be opened.")
                    return False
